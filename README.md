# Douding 机器人 GIF 表情组件

Douding Robot Emoji GIF Component for ESP-IDF
...
void switch_emotion(lv_obj_t* gif, const char* emotion_name) {
    const lv_img_dsc_t* emotion = douding_emoji_gif_get_by_name(emotion_name);
    if (emotion != NULL) {
...
void print_component_info() {
    printf("douding Emoji GIF组件版本: %s\n", douding_emoji_gif_get_version());
    printf("支持的表情数量: %d\n", douding_emoji_gif_get_count());
}
```
...
```
douding-emoji-gif-component/
├── idf_component.yml          # 组件配置文件
├── CMakeLists.txt            # 构建配置
├── README.md                 # 说明文档
├── LICENSE                   # 许可证
├── include/
│   └── douding_emoji_gif.h      # 头文件
└── src/
    ├── douding_emoji_gif_utils.c # 辅助函数
